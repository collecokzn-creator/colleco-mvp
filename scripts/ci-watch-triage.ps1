param(
  [int]$minutes = 15,
  [string]$branch = 'feat/siteminder-ci-mock',
  [string]$repo = 'collecokzn-creator/colleco-mvp'
)

Write-Host "Starting CI watch/triage for branch $branch (duration: $minutes min)"
$end = (Get-Date).AddMinutes($minutes)
New-Item -ItemType Directory -Path artifacts/triage -Force | Out-Null

while((Get-Date) -lt $end) {
  Write-Host "Checking runs at $(Get-Date -Format o) ..."
  $runsJson = gh run list --repo $repo --branch $branch --limit 50 --json databaseId,workflowName,conclusion,createdAt 2>&1 | Out-String
