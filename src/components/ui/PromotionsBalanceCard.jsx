  ...
  return (
    <Link to={to} className="block rounded border border-brand-gold bg-white p-3 hover:border-brand-orange transition text-brand-orange">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="font-semibold text-brand-orange">Promotions Balance</span>
      </div>
      <div className="text-xl font-bold text-brand-gold">{currency} {balance.toLocaleString()}</div>
      <div className="flex items-center gap-1 text-sm mt-1 text-brand-gold">
        <CreditCard className="h-4 w-4 text-brand-gold" />
        <span>Top-up credits or set monthly budget</span>
      </div>
      <p className="text-[11px] text-brand-gold mt-2">Self-serve ad placements and featured listings are managed here.</p>
    </Link>
  );
}
